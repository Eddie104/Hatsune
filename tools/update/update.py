...
def createBundle(platform):
	...
	# 生成路径
	outDir = os.path.join(os.path.dirname(__file__), "%sbundle/%s/%s/" % (PROJECT_ROOT, targetVersion, platform))
	if not os.path.exists(outDir):
		os.makedirs(outDir)

	entryFile = 'index.%s.js' % platform;
	outDir = os.path.join(os.path.dirname(__file__), "bundle/%s/%s/" % (targetVersion, platform))
	bundleFile = '%sindex.%s.bundle' % (outDir, platform)

	if sysPlatform.system() == 'Darwin':
		os.system('cd %s; react-native bundle --entry-file %s --bundle-output %s --platform %s --assets-dest %s --dev false' % (PROJECT_ROOT, entryFile, bundleFile, platform, outDir))
	else:
	...
	print('bundle create success!version => %s platform => %s' % (targetVersion, platform))

def createJSPatches(prevJSPath, curJSPath, patchPath):
	os.system('bsdiff %s %s %s' % (prevJSPath, curJSPath, patchPath))
...
def createPatchesBetween(prevVersion, curVersion, platform, updateJson):
	...
	# 找出上一个bundle
	prevBundleFiles = BundleFiles(r"%s/bundle/%s/%s" % (PROJECT_ROOT, prevVersion, platform), platform)
	# 找出当前的bundle
	bundleFileDir = "%s/bundle/%s/%s" % (PROJECT_ROOT, curVersion, platform)
...
def createPatch(platform):
	...
	versionArr = []
	bundleDir = os.path.join(os.path.dirname(__file__), "%sbundle/" % PROJECT_ROOT)
...
def upload(platform):
	# ftp = myFtp('123.207.140.58')
	# ftp.Login('easyftp','xrh-7zWv')
	# try:
	# 	# 先上传update.json
	# 	ftp.UpLoadFile("%s/update.%s.json" % (PROJECT_ROOT, platform), '/update/easy/%s/update.%s.json' % (platform, platform))
	# 	# 再上传zip包
	# 	patchDir = os.path.join(os.path.dirname(__file__), "%spatches/%s/" % (PROJECT_ROOT, platform))
	# 	for parent, dirNames, fileNames in os.walk(patchDir):
	# 		for fileName in fileNames:
	# 			ftp.UpLoadFile(os.path.join(parent, fileName), '/update/easy/%s/%s' % (platform, fileName))
	# except Exception, e:
	# 	err(u"上传到ftp出错了")
	# finally:
	# 	ftp.close()
